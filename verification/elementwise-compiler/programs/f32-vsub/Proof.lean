...
private theorem neonBlock4_eq_zipWith (p : f32vsubParams)
  ...
  have hFn : fNeon p = SALT.Intrinsics.FP32.sub := by
    funext x y
    simp [fNeon, neonBlock4FromIntrinsics, SALT.Intrinsics.Neon.vsubq_f32]
  simp [neonBlock4FromIntrinsics, SALT.Intrinsics.Neon.vsubq_f32,
    hTakeA, hTakeB, hFn]

private theorem rvvChunk_eq_zipWith (p : f32vsubParams)
    (inputA inputB : List (BitVec 32)) :
    rvvChunkFromIntrinsics p inputA inputB =
      List.zipWith (fRvv p) inputA inputB := by
  have hFn : fRvv p = SALT.Intrinsics.FP32.sub := by
    funext x y
    simp [fRvv, rvvChunkFromIntrinsics, SALT.Intrinsics.RVV.vfsub_vv_f32]
  simp [rvvChunkFromIntrinsics, SALT.Intrinsics.RVV.vfsub_vv_f32, hFn]
...
private theorem elementFunctionsEqual : elementFunctionsEqualClaim := by
  ...
  simp [fNeon, fRvv, neonBlock4FromIntrinsics, rvvChunkFromIntrinsics,
    SALT.Intrinsics.Neon.vsubq_f32, SALT.Intrinsics.RVV.vfsub_vv_f32]

private theorem neonPartialTail_eq_take_zipWith (p : f32vsubParams)
  ...
  rcases hCases with h | h | h | h <;> subst live <;>
    simp [neonPartialTailLivePrefixFromIntrinsics, fNeon,
      neonBlock4FromIntrinsics, SALT.Intrinsics.Neon.vsubq_f32, Nat.testBit]

private theorem neonTailWithOverread_eq_zipWith (p : f32vsubParams)
