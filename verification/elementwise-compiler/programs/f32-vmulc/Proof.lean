import SALT.Corpus.f32vmulc.Spec

namespace SALT.Corpus.f32vmulc

private theorem neonBlock4_eq_map (p : f32vmulcParams)
  ...
  have hFn : fNeon p = fun x => SALT.Intrinsics.FP32.mul x p.broadcast_input_b := by
    funext x
    simp [fNeon, neonBlock4FromIntrinsics, SALT.Intrinsics.Neon.vmulq_f32]
  simp only [neonBlock4FromIntrinsics, SALT.Intrinsics.Neon.vmulq_f32, hTake]
  rw [SALT.zipWith_replicate_right SALT.Intrinsics.FP32.mul p.broadcast_input_b
    input 4 (by simpa using hLength.symm)]
  rw [hFn]

private theorem rvvChunk_eq_map (p : f32vmulcParams)
    (input : List (BitVec 32)) :
    rvvChunkFromIntrinsics p input = input.map (fRvv p) := by
  have hFn : fRvv p = fun x => SALT.Intrinsics.FP32.mul x p.broadcast_input_b := by
    funext x
    simp [fRvv, rvvChunkFromIntrinsics, SALT.Intrinsics.RVV.vfmul_vf_f32]
  simp [rvvChunkFromIntrinsics, SALT.Intrinsics.RVV.vfmul_vf_f32, hFn]
...
private theorem elementFunctionsEqual : elementFunctionsEqualClaim := by
  ...
  simp [fNeon, fRvv, neonBlock4FromIntrinsics, rvvChunkFromIntrinsics,
    SALT.Intrinsics.Neon.vmulq_f32, SALT.Intrinsics.RVV.vfmul_vf_f32]

private theorem neonPartialTail_eq_take_map (p : f32vmulcParams)
  ...
  rcases hCases with h | h | h | h <;> subst live <;>
    simp [neonPartialTailLivePrefixFromIntrinsics, fNeon,
      neonBlock4FromIntrinsics, SALT.Intrinsics.Neon.vmulq_f32, Nat.testBit]

private theorem neonTailWithOverread_eq_map (p : f32vmulcParams)
