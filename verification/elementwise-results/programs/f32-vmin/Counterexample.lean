import SALT.Corpus.f32vmin.Spec

namespace SALT.Corpus.f32vmin

def counterexampleParams : f32vminParams := ({  } : f32vminParams)
def counterexampleInput0 : List (BitVec 32) := [0]
def counterexampleInput1 : List (BitVec 32) := [2143289344]
...
example : ((neonValueLoopWithOverreadFromIntrinsics counterexampleParams counterexampleInput0 counterexampleInput1 counterexampleOverread0 counterexampleOverread1 (by decide : counterexampleInput0.length = counterexampleInput1.length)).headD 0).toNat = 2143289344 := by
  native_decide
example : ((rvvValueLoopFromIntrinsics counterexampleParams counterexampleInput0 counterexampleInput1 (by decide : counterexampleInput0.length = counterexampleInput1.length) counterexampleSchedule).headD 0).toNat = 0 := by
  native_decide
