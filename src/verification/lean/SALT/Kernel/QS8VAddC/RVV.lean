/- RVV kernel model for qs8-vaddc (scalar-broadcast, via List.map).
...
/-- RVV pipeline for one input byte.
...
/-- RVV iteration built from list-level intrinsic combinators, mirroring
    kernels/target/qs8-vaddc.c:26-48. -/
...
/-- The intrinsic-composed pipeline equals `List.map rvvElemFn`. -/
theorem rvvPipeline_eq_map (p : QS8AddMinmaxParams) (bias : BitVec 32)
  ...
  simp_core_unfold [vwsub_vx, vsext_vf2, vmacc_vx, vssra_vx_rnu,
    vnclip_wx_i16, vnclip_wx_i8, vsadd_vx, vmax_vx, vmin_vx,
    zipWith_replicate_left, zipWith_replicate_right,
...
/-- One iteration equals `List.map (rvvElemFn p bias)`. -/
theorem rvvIteration_eq_map (p : QS8AddMinmaxParams) (bias : BitVec 32)
    (chunk : List (BitVec 8)) :
    rvvIteration p bias chunk = chunk.map (rvvElemFn p bias) :=
  rvvPipeline_eq_map p bias chunk

/-- RVV loop: min(remaining, vlmax) elements per iteration; vl adjusts to the
    remaining length, so no tail handling is needed.
    Models the C loop `for (; batch > 0; ) { vl = vsetvl(batch); ... }`. -/
