...
namespace SALT.Kernel.QS8VAddCParsedFully.RVV
...
def rvvElemFn (p : QS8AddMinmaxParams) (bias : BitVec 32) (x : BitVec 8) : BitVec 8 :=
  let vxa : BitVec 16 := ((sext x 16) - (sext p.a_zero_point 16))
  let vxa32 : BitVec 32 := (sext vxa 32)
  let vacc : BitVec 32 := (bias + (vxa32 * p.a_multiplier))
  let vacc : BitVec 32 := ((BVShiftOp.roundShr .rvvRnu).eval vacc p.shift.toNat)
  let vacc16 : BitVec 16 := (signedClamp vacc 16)
  let vacc16 : BitVec 16 := (signedSatAdd vacc16 p.output_zero_point)
  let vout : BitVec 8 := (signedClamp vacc16 8)
  let vout : BitVec 8 := (bvSignedMax vout p.output_min)
  (bvSignedMin vout p.output_max)
...
def rvvPipelineFromIntrinsics (p : QS8AddMinmaxParams) (bias : BitVec 32) (chunk : List (BitVec 8)) : List (BitVec 8) :=
  ...
  let vbias_vec := List.replicate chunk.length bias
  let vacc := vmacc_vx vbias_vec p.a_multiplier vxa32
    (h := by simp [vbias_vec, vxa32, vxa])
...
end SALT.Kernel.QS8VAddCParsedFully.RVV
