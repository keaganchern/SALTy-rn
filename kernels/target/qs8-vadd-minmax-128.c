    ...
    const struct xnn_qs8_add_minmax_params* restrict params) XNN_OOB_READS
{
  ...
  size_t vl;
  for (; batch > 0; batch -= vl, input_a += vl, input_b += vl, output += vl) {
    // Use LMUL=2 for int8 to allow widening up to LMUL=8 for int32
    vl = __riscv_vsetvl_e8m2(batch);

    vint8m2_t va = __riscv_vle8_v_i8m2(input_a, vl);
    vint8m2_t vb = __riscv_vle8_v_i8m2(input_b, vl);

    // Widen int8 -> int16 and subtract zero points
    ...
    // Multiply and accumulate: vacc = vxa32 * a_multiplier + vxb32 * b_multiplier
