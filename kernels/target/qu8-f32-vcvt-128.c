    ...
    const struct xnn_qu8_f32_cvt_params* restrict params) XNN_OOB_READS
{
  ...
  const int16_t minus_zero_point = (int16_t)(-params->scalar.zero_point);
  ...
  while (batch > 0) {
    ...
    vint16m4_t vx_i16 = __riscv_vreinterpret_v_u16m4_i16m4(vx_u16);
    vint16m4_t vhx = __riscv_vadd_vx_i16m4(vx_i16, minus_zero_point, vl);
